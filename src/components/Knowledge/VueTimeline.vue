...
export default {
  ...
  methods: {
    ...
    //鼠标移入结点，同id的tail和node变颜色
    cardMouseEnter(e) {
      var cardId = e.target.id;
      var valueId = cardId.split("-")[1];
      //不断记录悬停时候的valueId，因为单独给card加上点击事件无法达到，层级太多，没有造成点击穿透
      //可以采用pointer-events: none给表层的div添加属性，但是太麻烦了，采用一个悬浮变量，一个点击变量，配合使用
      this.hoverDivIndex = valueId;
      //悬停css变换条件
      //1.此时没有点击任何结点      currentClickedId===-1
      //2.点击结点并非鼠标悬停结点  currentClickedId!=valueId
      if (
    ...
    cardMouseLeave(e) {
      ...
      this.hoverDivIndex = -1; //鼠标移出，清除id
    },
    cardClickEvent(e) {
      //这里偷了个巧，点击的前提是要先悬浮进去，于是悬浮的时候不断记录valueId即可
      var valueId;
      //保证悬停在card上
